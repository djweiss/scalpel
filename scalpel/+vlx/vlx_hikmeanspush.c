...
xcreate (VlHIKMTree *tree, mxArray const *mnode, int i)
{
  ...
  /* has any childer? */
  if (msub && mxGetNumberOfElements(msub) > 0) {

    /* sanity checks */
    if (mxGetClassID (msub) != mxSTRUCT_CLASS) {
        char buf[1000];
        sprintf(buf, "node.sub size is %d\n", mxGetNumberOfElements(msub));
      mexErrMsgTxt(buf); //"NODE.SUB must be a MATLAB structure array.") ;
    }
