function [image2,A] = imresize_nearest(image,newsize)
%{
% Timothee Cour, 06-Jun-2008 05:41:57 -- DO NOT DISTRIBUTE

image2=imresize_nearest(image,[p,q]);
image2=imresize_nearest(image,ratio);
%}

if numel(newsize) == 1
    newsize = round(size2(image,[1 2])*newsize);
end

image2=mex_imresize(image,newsize);
pq=size_pq(image);
pq2=size_pq(image2);
% A=computeTransformMatrix(0,[],1,pq/2);
A=register_rhulls(size2rhull(pq),size2rhull(pq2));

return;

% function varargout = imresize_nearest(A,m)
% imresize using 'nearest'
% modified to go faster when called on a lot of inputs
% skips input checking
% syntax : image2 = imresize_nearest(image1,[p,q])
%
